...
async fn run(app: AndroidApp) {
    ...
    while !quit {
                        ...
                        MainEvent::InitWindow { .. } => {
                            ...
                            if let Some(activeplayer) = &playerbox {
                                ...
                                if let Some(bytes) = bytes {
                                    ...
                                    // Load official AQW loader with correct base parameters
        ...
        match receiver.try_recv() {
            ...
            Ok(RuffleEvent::TextInput { codepoint }) => {
                if let Some(player) = playerbox.as_ref() {
                    player
                        .player
                        .lock()
...
#[no_mangle]
pub unsafe extern "C" fn Java_rs_ruffle_PlayerActivity_nativeOnTextInput(
    mut env: JNIEnv,
    this: JObject,
    text: JString,
) {
    let input: String = env.get_string(&text).expect("Couldn't get string").into();

    let event_loop: MutexGuard<Sender<RuffleEvent>> = env.get_rust_field(this, "eventLoopHandle").unwrap();

    for c in input.chars() {
        let _ = event_loop.send(RuffleEvent::TextInput { codepoint: c });
    }
}

#[no_mangle]
pub unsafe extern "C" fn Java_rs_ruffle_PlayerActivity_nativeOnBackspace(
    mut env: JNIEnv,
    this: JObject,
) {
